...
unsigned int nonce_table16[] = {0x0000,0x8005,0x800f,0x000a,0x801b,0x001e,0x0014,0x8011,0x8033,
...
SubMessageSeed::SubMessageSeed()
{
   
}

void SubMessageSeed::SetLotTid(unsigned long TheLot,unsigned long TheTid)
{
    Lot=TheLot;
    Tid=TheTid;
}

void SubMessageSeed::SetFromSubMessage(SubMessage *submessage_in,int SeqMessage)
{
    SeqMessagePOD=SeqMessage;
    submessage.Len=submessage_in->Len;
    memcpy(submessage.Body,submessage_in->Body,submessage.Len);
}


int SubMessageSeed::InterpertSubmessage()
{

    
    if(submessage.Len==3)
    {
                            Type=submessage.Body[0];
                                                        
                            if(Type==0x14) // Sync Seed
                            {
                                unsigned int Crc=0;
                                if(SeqMessagePOD==0) SeqMessagePOD=16;  // PDM request is seq 15                
                                Crc=nonce_table16[SeqMessagePOD-1];//Index-1 ??? Byte FA is byte1 -> This is the SequenceMessage from the PDM (means actual POD seq-1) 
                                unsigned int Result=((unsigned int)(submessage.Body[1])<<8)|submessage.Body[2];
                                unsigned int LotTid=(unsigned int)(Lot&0xFFFF)+(unsigned int)(Tid&0xFFFF);
                                //fprintf(stderr,"Seed request with seq=%d : \n",SeqMessagePOD);
                                for(int SearchSeed=0;SearchSeed<128;SearchSeed++)
                                {
                                        if(((LotTid+Crc+0x72AA)&0xFFFF)==(Result^SearchSeed)) //Fixme 0x72AA is unique fake nounce generated by PDM
                                        {
                                            Seed=SearchSeed;
                                            return 0; // Found the new Seed
                                        }
                                    
                                }
                            }
                            else 
                                fprintf(stderr,"Unknown Seed Type\n");
                                
                                
    }
    else 
    {    
        printf("SubMessageSeed incorrect len:%d\n",submessage.Len);
        return -1;
    }
    return -1;
...
int SubMessageSeed::PrintState()
{
    fprintf(stderr,"LotId %ld Tid %ld Seed = %d \n",Lot,Tid,Seed);                    
    return 0;
}
